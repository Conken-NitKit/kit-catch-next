...
const SubmitButton = styled.button<{ submittable: boolean }>`
  ...
  padding: 10px 0;
  border-radius: 3px;
  background: rgba(0, 149, 246, ${({ submittable }) => submittable ? 1 : .3
  });
  transform: translate(${({ submittable }) => submittable
    ? '-1px, -1px'
    : '0, 0'
  });
  &:active {
...
export default function ClassRadio() {
  ...
  return (
    <FormContainer>
      <ClassContainer>
        <ClassItem selected={grade === 1} onClick={() => setGrade(1)}>1年</ClassItem>
        <ClassItem selected={grade === 2} onClick={() => setGrade(2)}>2年</ClassItem>
        <ClassItem selected={grade === 3} onClick={() => setGrade(3)}>3年</ClassItem>
        <ClassItem selected={grade === 4} onClick={() => setGrade(4)}>4年</ClassItem>
        <ClassItem selected={grade === 5} onClick={() => setGrade(5)}>5年</ClassItem>
      </ClassContainer>
      ...
      <PasswordInput type={'password'} placeholder={'パスワード'} onChange={e => setPassword(e.target.value)} />
      <SubmitButton
